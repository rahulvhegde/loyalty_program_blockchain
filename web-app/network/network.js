...
module.exports = {
 ...
 sendPointsTransaction: async function (cardId, accountNumber, phoneNumber, points) {
  ...
  try {
    ...
    let sendPoints = {};
    sendPoints.points = points;
    sendPoints.member = accountNumber;
    sendPoints.phoneNumber = phoneNumber;
 ...
 allMembersInfo : async function (cardId) {
  ...
  try {
    ...
    let allMembers = await contract.evaluateTransaction('GetState', 'all-members');
  ...
  * @param {String} cardId Card id to connect to network
  */
 sendPointsTransactionsInfo: async function (cardId, userType, userId) {
  ...
  try {
    ...
    console.log(`\nGet send points transactions state for ${userType} ${userId}`);
    let sendPointsTransactions = await contract.evaluateTransaction('SendPointsTransactionsInfo', userType, userId);
    sendPointsTransactions = JSON.parse(sendPointsTransactions.toString());
    console.log(sendPointsTransactions);

    // Disconnect from the gateway.
    await gateway2.disconnect();

    return sendPointsTransactions;
  }
  catch(err) {
    //print and return error
    console.log(err);
